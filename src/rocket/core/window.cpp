    ...
    void window::cpl_init(window_backend_t bk) {
        switch (bk) {
            case rocket::window_backend_t::null:
                rocket::window::null_cpl_init();
            case rocket::window_backend_t::glfw:
                rocket::window::glfw_cpl_init();
            case rocket::window_backend_t::embedded:
                window::cpl_init(window_backend_t::null); // [FIXME] Add Embedded or remove it entirely
            default: if (cpl_init_callback != nullptr) cpl_init_callback(bk);
        }
    }
