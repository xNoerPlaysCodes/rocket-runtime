...
namespace rocket {
    ...
    void *plugin_t::get_function(std::string name) {
        if (this->handle == nullptr) {
            rocket::log_error("plugin not loaded", "plugin_t::get_function", "error");
            return nullptr;
        }
    ...
    plugin_capabilities_t *handle_init_plugin(lib_handle handle) {
        ...
        if (on_load == nullptr || on_unload == nullptr) {
            rocket::log_error("failed to get on_load or on_unload", "plugin.cpp::handle_init_plugin", "error");
            return nullptr;
        }

        plugin_capabilities_t *cap = on_load(api);
        if (cap == nullptr) {
            rocket::log_error("failed to get plugin capabilities", "plugin.cpp::handle_init_plugin", "error");
            return nullptr;
        }
        ...
        if (cap->needs_frame_events) {
            ...
            if (on_framestart == nullptr || on_frameend == nullptr) {
                rocket::log_error("failed to get on_framestart or on_frameend while capabilities::needs_frame_events is true", "plugin.cpp::handle_init_plugin", "error");
                return nullptr;
            }
        }

        if (cap->is_lazy_loadable) {
            rocket::log_error("lazy loadable plugins not supported", "plugin.cpp::handle_init_plugin", "error");
        }
    ...
    std::shared_ptr<plugin_t> load_plugin(std::filesystem::path plugin) {
        ...
        if (!std::filesystem::exists(plugin)) {
            rocket::log_error("plugin does not exist", "plugin.cpp::load_plugin", "error");
            return nullptr;
        }
        ...
        if (!mz_zip_reader_init_file(&zip, plugin.string().c_str(), 0)) {
            rocket::log_error("failed to open plugin", "plugin.cpp::load_plugin", "error");
            return nullptr;
        }
        ...
        for (int i = 0; i < num_files; ++i) {
            ...
            if (!mz_zip_reader_file_stat(&zip, i, &stat)) {
                rocket::log_error("failed to get file stats", "plugin.cpp::load_plugin", "error");
                return nullptr;
            }

            std::string filename = stat.m_filename;
            if (filename == "plugin.json") {
                rocket::log_error("[fixme] read plugin.json", "plugin.cpp::load_plugin", "info");
                ...
                if (!mz_zip_reader_extract_to_mem(&zip, i, buffer.data(), lib_size, 0)) {
                    rocket::log_error("failed to extract library from plugin", "plugin.cpp::load_plugin", "error");
                    return nullptr;
                }
                ...
                if (handle == nullptr) {
                    rocket::log_error("failed to load extracted library", "plugin.cpp::load_plugin", "error");
                    return nullptr;
                }
