...
namespace rocket {
    ...
    namespace io {
        ...
        bool key_down(keyboard_key key) {
            ...
            for (auto &kev : ::util::get_simulated_kevents()) {
                if (kev.key == key && kev.state.down()) return true;
            }
            return cur_actual;
#else
            (void)key; return false;
        ...
        bool key_released(keyboard_key key) {
            ...
            for (auto &kev : ::util::get_simulated_kevents()) {
                if (kev.key == key && kev.state.released()) return true;
            }
            return cur_actual;
#else
            (void)key; return false;
...
#ifdef ROCKETGE__Platform_Desktop
            if (state == keystate_t::make_down()) return key_down(key);
            else if (state == keystate_t::make_pressed()) return key_pressed(key);
            else if (state == keystate_t::make_released()) return key_released(key);
            rocket::log("[fixme] unimplemented key_state", "rocket::io", "key_state", "fixme");
            return false;
#else
            (void)key; (void)state; return false;
#endif
        }
        ...
        bool mouse_pressed(mouse_button button) {
            ...
            for (auto &mev : ::util::get_simulated_mevents()) {
                if (mev.button == button && mev.state.pressed()) return true;
            }
            return cur_actual;
        }
        ...
        bool mouse_down(mouse_button button) {
            ...
            for (auto &mev : ::util::get_simulated_mevents()) {
                if (mev.button == button && mev.state.down()) return true;
            }
            return cur_actual;
        }
        ...
        bool mouse_released(mouse_button button) {
            ...
            for (auto &mev : ::util::get_simulated_mevents()) {
                if (mev.button == button && mev.state.released()) return true;
            }
            return cur_actual;
        }
    ...
    namespace gpad {
        ...
        std::string get_human_readable_name(button_t button, style_t style) {
            ...
            if (it != button_names.end()) return it->second;
            return "Unknown";
        }
        ...
        std::string get_human_readable_name(axis_t axis, style_t style) {
            ...
            if (it != axis_names.end()) return it->second;
            return "Unknown";
        }
        ...
        void set_vibration(gamepad_t gp, float strength, float duration_ms) {
            ...
            if (gc == nullptr || haptic == nullptr) return;
            SDL_HapticRumblePlay(haptic, strength, static_cast<int>(duration_ms));
#else
            (void)gp; (void)strength; (void)duration_ms;
#endif
        }
    }
}
