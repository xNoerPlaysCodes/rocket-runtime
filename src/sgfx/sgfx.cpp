...
namespace sgfx {
    ...
    glm::mat4 build_model(const transform_t &t) {
        ...
        model = translate(model, glm::vec3(t.position.x, t.position.y, t.position.z));
    ...
    void renderer_t::end_frame() noexcept {
        for (const auto &obj : this->drawcalls) {
            ...
            glm::mat4 transform = build_model(obj.transform);
            ...
            if (this->ctx.unique_objects.find(obj.id) == this->ctx.unique_objects.end()) {
                ...
                shader = compile_shader(obj.draw_obj.shader.vertex_src.c_str(), obj.draw_obj.shader.fragment_src.c_str());
                gpu_object_t shader_obj = allocate_id();
                this->ctx.gl_objects[shader_obj] = gl::object_t {
            ...
            for (const auto &param : obj.shader_params) {
                ...
                if (loc != -1) {
                    std::visit([shader, loc, &param](auto&& v) {
            ...
            float color[4] = { color_nm.x, color_nm.y, color_nm.z, color_nm.w };
            if (u_color != -1)
    ...
    renderer_t::renderer_t(context_t &ctx, std::function<vec2i_t()> framebuffer_size_poller) : ctx(ctx) {
    ...
    void tx_allocator_init() {
        std::fill(texture_unit_pool.freelist, texture_unit_pool.freelist + 32, true);
        texture_unit_pool.max_idx = 32;
    }
